...
@Component({
    selector   : 'helix-shortcuts',
    templateUrl: './shortcuts.component.html',
    styleUrls  : ['./shortcuts.component.scss']
})
export class HelixShortcutsComponent implements OnInit, OnDestroy
{
    shortcutItems: any[];
    navigationItems: any[];
    ...
    ngOnInit(): void
    {
        ...
        this.filteredNavigationItems = this.navigationItems = this._helixNavigationService.getFlatNavigation(this.navigation);

        if ( this._cookieService.check('HELIX2.shortcuts') )
        {
            this.shortcutItems = JSON.parse(this._cookieService.get('HELIX2.shortcuts'));
        }
        else
        {
    ...
    toggleShortcut(event, itemToToggle): void
    {
        ...
        for ( let i = 0; i < this.shortcutItems.length; i++ )
        {
            if ( this.shortcutItems[i].url === itemToToggle.url )
            {
                ...
                // Save to the cookies
                this._cookieService.set('HELIX2.shortcuts', JSON.stringify(this.shortcutItems));
